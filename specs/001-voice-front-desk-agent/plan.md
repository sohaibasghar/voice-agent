...
**Technical approach** (from research.md): browser runs an OpenAI Agents SDK `RealtimeSession` over **direct WebRTC** to OpenAI for lowest latency; NestJS mints short-lived ephemeral tokens (real key stays server-side) and owns all business logic, the SQLite calendar store, and the authoritative guardrails (no-double-book, confirm-before-destructive) as injectable providers. Browser-side tools are thin proxies to `/tools/*`; session events (tool calls, handoffs, guardrail trips) relay to NestJS over `/realtime` WS to build the inspectable trace view (the hosted dashboard is not automatic for browser sessions — see research D3/D9). Three agents — FrontDesk (triage) ⇄ Services ⇄ Escalation — via handoffs. The audio transport is isolated so Twilio Media Streams can replace WebRTC later without touching agents/tools/guardrails/data model.
