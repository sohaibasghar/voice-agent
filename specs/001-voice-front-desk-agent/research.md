...
## D3. Session topology — DECISION (resolves the WebRTC vs server-tools/trace conflict)

- **Decision**: **Direct browser WebRTC + event relay** (user-selected 2026-06-12). The `RealtimeSession` and all agents run in the **browser**, with a direct WebRTC media link to OpenAI. Tool `execute()` bodies are **thin proxies** that call authenticated NestJS HTTP endpoints; the real business logic, data, and the double-book/confirmation checks live in NestJS providers. The browser relays session events (tool calls, handoff decisions, guardrail trips) to NestJS over the `/realtime` WebSocket so the demo UI shows them live.
- **Rationale**: The PRD locks browser WebRTC for lowest latency and lowest demo-failure risk (§1, §16). With WebRTC the session must run browser-side (tools execute where the session runs). Keeping tool *logic* in NestJS providers preserves the PRD's "tools resolve server-side / injectable & testable" principle (§7) — the browser tool is a 5-line `fetch`. This avoids the audio-bridge plumbing the PRD calls the fiddliest, most demo-fragile part.
- **Consequence (tracing)**: The OpenAI **hosted** trace dashboard is **not** automatic for a browser session — trace ingestion needs the real key, which the browser lacks, and SDK tracing is off by default in browsers. We therefore satisfy the "inspectable trace" requirement (FR-015, SC-007) via the **relayed-event trace view**: NestJS records every tool call (args + result), handoff, and guardrail trip received over `/realtime`, persisted as a per-session trace and rendered in the UI. NestJS additionally emits server-side spans around tool execution so the hosted dashboard shows tool activity if `OPENAI_API_KEY` is set server-side and the SDK is used to wrap those calls. The on-stage "open the trace" beat uses the relayed-event view.
- **Alternatives considered**: Server-side session + audio bridge (rejected by user — gets automatic hosted traces and native server-side tools but reintroduces the demo-fragile media bridge and an extra audio hop).
