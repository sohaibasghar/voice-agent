...
export const BEAUTY_SALON_PRESET: DomainConfig = {
  ...
  agents: {
    frontDesk: {
      name: 'FrontDeskAgent',
      handoffDescription:
        'The salon front desk that books, reschedules, and cancels appointments.',
      instructions: [
        ...
        'TIMES: When speaking any appointment time, use the slot/booking "label" field (already in local salon time) — say it naturally like "Tuesday the 16th at 9 AM". NEVER read the raw startTime/ISO value and never mention UTC or timezones. Pass the slot\'s exact startTime back to bookSlot/rescheduleBooking unchanged.',
        ...
        'SERVICES, PRICING & FAQs: Answer these yourself — ALWAYS call lookupServices (for service, package, or pricing questions) or lookupFAQ (for hours, location, parking, payment, cancellation policy) first, then read back only what the tool returns. Prices are in cents (4500 = "$45"). Never invent prices, hours, or policies; if a lookup returns nothing, say you do not have that info. You may transfer to ServicesAgent only for a long, detailed package consultation.',
        'TRANSFERS: If the caller asks for a human or to leave a message, transfer to EscalationAgent. Politely deflect clearly off-topic requests back to booking, services, or FAQs.',
      ].join('\n'),
    },
    ...
    escalation: {
      ...
      handoffDescription:
        'Takes a message / logs a callback when a human is needed.',
      instructions:
  ...
  seed: {
    services: [
      ...
      // Packages
      {
        name: 'Bridal Package – Classic',
        category: 'Packages',
        durationMin: 180,
        price: 35000,
      },
      {
        name: 'Bridal Package – Luxe',
        category: 'Packages',
        durationMin: 240,
        price: 52000,
      },
      ...
      {
        name: 'Pamper Package (Mani + Facial)',
        category: 'Packages',
        durationMin: 90,
        price: 9000,
      },
